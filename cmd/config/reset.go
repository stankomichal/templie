...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		helpers.VerbosePrintln(cmd, "Starting config reset process")
		helpers.VerbosePrintf(cmd, "Variable name: %s\n", args[0])

		helpers.VerbosePrintf(cmd, "Resetting variable %s to default value\n", args[0])
		...
		helpers.VerbosePrintln(cmd, "Config reset process completed")
	},
}
