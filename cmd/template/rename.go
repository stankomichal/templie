...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		helpers.VerbosePrintln(ctx, "Starting template rename process")

		helpers.VerbosePrintf(ctx, "Raw old name: %s\n", args[0])
		oldName := helpers.SanitizeName(args[0])
		helpers.VerbosePrintf(ctx, "Sanitized old name: %s\n", oldName)

		helpers.VerbosePrintf(ctx, "Raw new name: %s\n", args[1])
		newName := helpers.SanitizeName(args[1])
		helpers.VerbosePrintf(ctx, "Sanitized new name: %s\n", newName)
		...
		helpers.VerbosePrintf(ctx, "Renaming template from %s to %s\n", oldName, newName)
		...
		helpers.VerbosePrintln(ctx, "Template rename process completed")
	},
}
